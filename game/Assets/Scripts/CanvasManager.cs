...
public class CanvasManager : MonoBehaviour
{
    ...
    {
        var chainId = await ContractManager.Instance.ChainId();

        if (!ContractManager.Instance.isCrossChainTransferSupportedFromChainId(chainId))
        {
            return;
        }
        string optionText = chainDropdown.options[chainDropdown.value].text;
        //if (!isCrossChainTransferSupportedFromChainName(optionText))
        //{
        //    return;
        //}
        (bool isCrossChainSupported, string destinationChainIdString) = ContractManager.Instance.GetChainIdFromChainNameCrossChainTransfer(optionText);
        if (!isCrossChainSupported)
        {
            return;
        }
        int destinationChainId = int.Parse(destinationChainIdString);
        ...
        try
        {
            await ContractManager.Instance.TransferUtilsCrossChain(destinationChainId, tokenId, amountInput.text);
        }
        catch (Exception)
        {
            HideTransferPanel();
    ...
    async void ShowTransferPanel()
    {
        ...
        var optionDatas = ContractManager.Instance.GetCCIPSupportedAddresses().ContainsValue(chainId) ? GetOptions(ContractManager.Instance.GetCCIPSupportedAddresses(), chainId) : GetOptions(ContractManager.Instance.GetLxLySupportedAddresses(), chainId);
        chainDropdown.AddOptions(optionDatas);
    }

    List<TMP_Dropdown.OptionData> GetOptions(Dictionary<string, string> all, string currentChainId)
    {
        var optionDatas = new List<TMP_Dropdown.OptionData>();
        foreach (KeyValuePair<string, string> kvp in all)
        {
            if (kvp.Value != currentChainId)
            {
                optionDatas.Add(new TMP_Dropdown.OptionData() { text = kvp.Key });
            }
        }
        return optionDatas;
    }
